...
export default function CustomCalendar({ onDateClick, events = [] }: Props) {
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          ...
          >
            {/* número do dia com borda roxa (somente hoje) */}
            ...
            {/* chips de eventos (3 no desktop, 2 no mobile) */}
            {eventsForDaySorted.slice(0, 2).map((event) => (
              <div
                key={event.id}
                className="mt-1 text-[10px] sm:text-[11px] leading-4 bg-[#672C8E] text-white rounded px-1.5 sm:px-2 py-0.5 sm:py-1 truncate max-w-full shadow-sm"
                title={event.title}
              >
                {event.title}
              </div>
            ...
            {eventsForDaySorted[2] && (
              <div
                ...
                className="hidden sm:block mt-1 text-[11px] leading-4 bg-[#672C8E] text-white rounded px-2 py-1 truncate max-w-full shadow-sm"
            ...
            {eventsForDaySorted.length > 3 && (
              <div className="mt-auto text-[#672C8E] text-xl sm:text-2xl font-bold mx-auto leading-tight">
                +{eventsForDaySorted.length - 3}
              </div>
            )}
          </motion.div>
  ...
  return (
    <div className="w-full bg-white rounded-xl sm:rounded-2xl shadow-sm ring-1 ring-black/5 p-3 sm:p-4">
      ...
      <div className="hidden sm:flex mt-3 items-center gap-3 text-xs text-gray-600">
